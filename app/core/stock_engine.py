...
def analyze_stock(symbol):
    ...
    try:
        ...
        result["piotroski_score"] = calc_piotroski_score(result)
    result["x_times_return"] = calc_x_return_potential(result)
    result["magic_formula"] = classify_magic_formula(result)
    return result
...
# ---------------------------
# Advanced stock scoring
# ---------------------------
def calc_piotroski_score(data):
    score = 0
    try:
        if data.get("roe") and data["roe"] > 0:
            score += 1  # ROE positive
        if data.get("peg") and data["peg"] < 1.5:
            score += 1  # Undervalued
        if data.get("cagr_eps") and data["cagr_eps"] > 15:
            score += 1  # Growing EPS
        if data.get("price_cagr") and data["price_cagr"] > 10:
            score += 1  # Price momentum
        if data.get("financials"):
            years = list(data["financials"].get("Profit", {}).keys())[-2:]
            profits = [safe_float(data["financials"]["Profit"].get(y, 0)) for y in years]
            if len(profits) == 2 and profits[1] > profits[0]:
                score += 1  # Increasing profit
        return score
    except:
        return score

def calc_x_return_potential(data):
    try:
        if data.get("intrinsic_value") and data.get("valuation_chart"):
            last_price = data["valuation_chart"]["price"][-1]
            return round(data["intrinsic_value"] / last_price, 2)
        return None
    except:
        return None

def classify_magic_formula(data):
    try:
        if data.get("peg") and data.get("roe"):
            if data["peg"] < 1.5 and data["roe"] > 20:
                return True
        return False
    except:
        return False
